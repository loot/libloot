...
which other groups, with groups being added in the order that they are listed
in the masterlist and then in the userlist. Once all the groups have been added,
a depth-first search is performed starting from each group in the order they
were added, except that groups that don't load after any other groups are
searched from first.

At the start of each search, the starting group is used as the first element in a
stack that will represent the current path through the graph. On each new edge
